...
class cityscapes:
    ...
    def list_label_frames(self, split):
        ...
        collected as (city, shot, idx) tuples (NOT ACCURATE)
        (MORE ACCURATE): frames is a list of elements like 'lindau_000000_000019'
        """
        def file2idx(f):
        ...
        frames = []
        cities = [os.path.basename(f) for f in glob.glob('{}/gtFine_trainvaltest/gtFine/{}/*'.format(self.dir, split))]
        for c in cities:
            files = sorted(glob.glob('{}/gtFine_trainvaltest/gtFine/{}/{}/*labelIds.png'.format(self.dir, split, c)))
